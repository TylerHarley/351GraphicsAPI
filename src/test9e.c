...
int main(int argc, char *argv[]) {
	int frame;
	Color blue, green, purple, red, white, surf, light;
	Point p[16];
	Point source;
	BezierSurface bc;
	DrawState ds;
	...
	color_set(&surf, .2, .2, .2);
	color_set(&light, .9, .9, .9);
	...
	// put the curve into a module
	module_bodyColor(curve, &green);
	module_surfaceColor(curve, &surf);
	module_surfaceCoeff(curve, 16);
	module_bezierSurface(curve, &bc, divisions, 1);
	...
	point_set3D(&source, 0.5, 1.7, -1.0); 
	lighting_add(lighting, LightPoint, &light, NULL, &source, 0.0, 0.0);
	lighting_add(lighting, LightAmbient, &surf, NULL, NULL, 0.0, 0.0);
	...
	for(frame=0;frame<60;frame++) {
		...
		printf("entering module draw\n");
		module_draw( curve, &VTM, &GTM, &ds, lighting, src );
		printf("finished module draw of curves\n");

		sprintf(buffer, "../images/surface/surface-frame%03d.ppm", frame);
