...
int main(int argc, char *argv[]) {
	...
	printf("done with curve B\n");

	// create a module with six curves
	curves = module_create();
	for(i=0;i<3;i++) {
		module_module( curves, curveA );
		printf("bezier curve A added to module curves\n");
		module_module( curves, curveB );
		printf("bezier curve B added to module curves\n");
	...
	for(frame=0;frame<60;frame++) {
		...
		printf("done drawing module with curves\n");
		sprintf(buffer, "../images/bez1/bez3d-frame%03d.ppm", frame);
